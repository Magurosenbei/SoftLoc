...
{
	outPosition = mul(matWorld, inPosition);
	outNormal	= mul(matWorld, inNormal);
	outTexCoord = inTexCoord;
	outLightDir = normalize(vLightPosition - outPosition).xyz;
...
out	float4	outColor	:	COLOR)
{
	...
	outColor.xyz = saturate(Ambient + Diffuse);
}